...
class VideoDataset(Dataset):
    def __init__(self, video_path, img_size=416, augment=True, multiscale=True, normalized_labels=True):
        ...
        self.batch_count = 0
        self.frame_count = 0
        self.lower_ybound = 600
        self.upper_ybound = 1750
        ...
        print("took", time.time() -ckpt_time)
    ...
    def __getitem__(self, index):
        ...
        print('getitem:', index)
        _, vis_img = self.cap.read()
        print(vis_img.shape)
        vis_img = vis_img[self.lower_ybound:self.upper_ybound,:,:]
        print(vis_img.shape)

        # img = transforms.ToTensor()(Image.open(img_path).convert('RGB'))
        img = cv2.cvtColor(vis_img, cv2.COLOR_BGR2RGB)
