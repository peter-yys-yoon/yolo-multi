    ...
    for sample_i in range(len(outputs)):
        ...
        if len(annotations):
            ...
            for pred_i, (pred_box, pred_label) in enumerate(zip(pred_boxes, pred_labels)):
                ...
                if pred_label.cpu() not in target_labels:
                    # pred_label tensor(cuda0)
                ...
                iou, box_index = bbox_iou(pred_box.unsqueeze(0).cpu(), target_boxes).max(0)
                # iou, box_index = bbox_iou(pred_box.unsqueeze(0), target_boxes.to(pred_box.get_device())).max(0)
                                                                                      # iou size: [gt_boxes_number]
...
def bbox_iou(box1, box2, x1y1x2y2=True):
    ...
    # pdb.set_trace()
    # if b1_x1.is_cuda or b1_x2.is_cuda:
    # print(type(b1_x1), b1_x1.get_device(), type(b2_x1), b2_x1.get_device())
